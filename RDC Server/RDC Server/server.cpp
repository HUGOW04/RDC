...
constexpr auto WINDOW_NAME = "RDC-Inviter";
            ...
            if (rdp_session->Open() == S_OK)
            {
                ...
                if (rdp_session->get_Invitations(&rdp_invitationManager) == S_OK)
                {
                    HRESULT resOfCreatingInvitation = rdp_invitationManager->CreateInvitation(
                        SysAllocString(L"RDControl"),        // String to use for the authorization.
                        SysAllocString(L"RDC-Control-Group"),  // The name of the group.
